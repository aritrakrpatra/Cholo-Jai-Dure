...
function getActiveItem(href, pathname, hash, category) {
  if (href.startsWith("/tours?category=")) {
    return pathname === "/tours" && category === href.split("category=")[1];
  }
...
function MoreMenuButton({ pathname, currentHash, isLightTheme, mobile = false, category }) {
  const [open, setOpen] = useState(false);
  const menuRef = useRef(null);
  const active = moreNavItems.some((item) => getActiveItem(item.href, pathname, currentHash, category));
      ...
      {open && (
        <div className={menuClasses}>
          {moreNavItems.map((item) => {
            const itemActive = getActiveItem(item.href, pathname, currentHash, category);
            return (
              <Link
                key={item.href}
                href={item.href}
        ...
        <div className="mx-auto flex w-full max-w-7xl items-center justify-between gap-3 lg:grid lg:grid-cols-[auto_minmax(0,1fr)_auto] lg:gap-4">
          ...
          <nav className={`hidden min-w-0 items-center justify-center gap-4 overflow-visible whitespace-nowrap text-sm font-medium lg:flex ${isLightTheme ? "text-slate-700" : "text-white/80"}`}>
            {desktopPrimaryNavItems.map((item) => {
              const active = getActiveItem(item.href, pathname, currentHash, category);

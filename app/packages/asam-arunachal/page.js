      ...
      <main className="min-h-screen bg-linear-to-br from-slate-950 via-slate-900 to-slate-950 text-white">
        <section className="relative overflow-hidden">
          <div
            ...
            style={{
              backgroundImage:
                "linear-gradient(to bottom, rgba(15,23,42,0.45), rgba(15,23,42,0.92)), url('/group2.jpeg')",
            }}
          />
          <div className="relative z-10 mx-auto max-w-7xl px-4 py-10 sm:px-6 sm:py-16 lg:py-20">
            <div className="grid gap-8 lg:grid-cols-[1.65fr_0.95fr] lg:items-end">
              <div className="space-y-4">
                ...
                <h1 className="text-4xl font-bold sm:text-5xl lg:text-6xl">Assam &amp; Arunachal Pradesh</h1>
                <p className="max-w-3xl text-base leading-7 text-white/80 sm:text-lg sm:leading-8 md:text-xl">
                  A mountain-rich March route from Assam into Arunachal Pradesh covering Bhalukpong, Dirang, Tawang, and Bomdilha.
                </p>
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <span className="rounded-full border border-white/10 bg-white/5 px-4 py-2 text-xs sm:px-5 sm:py-3 sm:text-sm">6 Nights · 7 Days · March Batch</span>
                </div>
                <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:gap-3">
                  <BookNowButton packageName="Assam & Arunachal Pradesh" packageId="asam-arunachal" />
                ...
                <div className="space-y-4">
                  <div className="rounded-4xl border border-amber-300/10 bg-slate-950/90 p-5">
                    ...
                    <div className="mt-5 space-y-3 text-sm text-white/80">
                      <p><span className="font-semibold text-white">Journey:</span> 10/03/2027</p>
                      <p><span className="font-semibold text-white">Return:</span> 17/03/2027</p>
                      <p><span className="font-semibold text-white">Route:</span> Guwahati → Bhalukpong → Dirang → Tawang → Bomdilha → Return</p>
                    </div>
                  </div>
                  <div className="rounded-4xl border border-white/10 bg-slate-950/90 p-5">
                    ...
                    <p className="mt-4 text-2xl font-semibold text-white">Northeast route coordination with full support.</p>
                  </div>
                </div>
              </aside>
            </div>
        ...
        <section className="py-10 sm:py-14">
          <div className="mx-auto max-w-7xl px-4 sm:px-6">
            <div className="mb-12 text-center">
              ...
              <h2 className="mt-4 text-2xl font-bold sm:text-4xl">Your March journey day by day</h2>
              <p className="mx-auto mt-5 max-w-3xl text-base leading-8 text-white/70 sm:text-lg">Follow a scenic ascent from Assam plains to Arunachal's highland monasteries and valleys.</p>
            </div>
            <div className="space-y-10">
              {itinerary.map((item) => (
                <div key={item.day} className="grid gap-6 rounded-4xl border border-white/10 bg-slate-900/90 p-8 lg:grid-cols-[140px_1fr] lg:items-start">
                  <div className="rounded-3xl bg-slate-950/80 p-4 text-center"><p className="text-xs uppercase tracking-[0.35em] text-amber-300">{item.day}</p></div>
                  <div>
                    <h3 className="text-xl font-bold text-white">{item.title}</h3>
                    <ul className="mt-3 space-y-2 text-white/70">
                      {item.points.map((point) => (
                        <li key={point} className="flex gap-2 text-sm leading-7"><span className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-amber-300" /><span>{point}</span></li>
                      ))}
                    </ul>
                  </div>
                </div>
            ...
            <div className="rounded-4xl border border-white/10 bg-slate-900/90 p-5 shadow-2xl sm:p-8 lg:p-10">
              <div className="grid gap-6 sm:grid-cols-2 sm:gap-10">
                <div>
                  ...
                  <ul className="mt-4 space-y-2 text-sm text-white/80">{inclusions.map((item) => (<li key={item} className="flex items-start gap-2 rounded-2xl bg-slate-950/80 px-3 py-2"><span className="mt-1 h-2 w-2 rounded-full bg-emerald-300" /><span>{item}</span></li>))}</ul>
                </div>
                <div>
                  <h2 className="text-xl font-bold sm:text-2xl">Exclusions</h2>
                  <ul className="mt-4 space-y-2 text-sm text-white/80">{exclusions.map((item) => (<li key={item} className="flex items-start gap-2 rounded-2xl bg-slate-950/80 px-3 py-2"><span className="mt-1 h-2 w-2 rounded-full bg-red-400" /><span>{item}</span></li>))}</ul>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="rounded-3xl border border-white/10 bg-slate-900/90 p-5 shadow-2xl sm:p-8 lg:p-10">
              ...
              <div className="mt-6 grid grid-cols-2 gap-3 sm:mt-8 sm:gap-4 lg:grid-cols-4">
                {galleryItems.map((item) => (
                  <div key={item.label} className="overflow-hidden rounded-[1.25rem] bg-white/5 sm:rounded-[1.75rem]"><Image src={item.src} alt={item.label} width={800} height={1000} className="h-36 w-full object-cover transition duration-500 hover:scale-105 sm:h-52 lg:h-64" /></div>
                ))}
              </div>
            </div>
          </div>
